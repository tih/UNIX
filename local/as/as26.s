...
opl37:
	cmp	r4,$'\n		/an expression?
	beq	9f		/no
	jsr	pc,expres	/get the start address
	tstb	passno		/ignore the 2nd (relocation) pass
	bne	9f
	bit	$1,r2		/odd?
	beq	0f		/no. ok
	jsr	r5,error; 'o
	br	9f
0:
	bic	$40,r3		/check addressing mode
	cmp	$2,r3		/text?
	beq	2f		/yes
	cmp	$3,r3		/data?
	beq	1f		/yes (too generous?)
	jsr	r5,error; 'a
	br	9f
1:
	inc	godata		/tell that start adr is in data
2:
	bis	$1,r2		/even if zero.
	mov	r2,stksiz	/and save it
9:
	tst	(sp)+		/fix stack
	mov	$'\e,savop	/force eof
	rts	pc
