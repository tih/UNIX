 ...
 * sufficient to get a running driver from the stock RP06
...
struct buf *abp;
{
	...
	p2 = lrem(p1, NSECTORS);
	p1 = ldiv(p1, NSECTORS);
	bp->trksec = (p1%NSURFACES)<<8 | p2;
	bp->cylin =+ p1/NSURFACES;
	spl5();
...
# ifdef XBUF
			*ka5 = praddr;
# endif
			if(u.u_procp->p_pri <= -5) {	/* priority req */
...
# endif
	if (HMADDR->hmcs1 & ERR) {		/* error bit */
		...
		spl0();
		deverror(bp, HMADDR->hmcs2);
/* expanded error dump for RM03 */
printf("cs1:%o ds:%o er1:%o er2:%o cs3:%o trksec:%o cyl:%o\n",
 HMADDR->hmcs1, HMADDR->hmds, HMADDR->hmer1, HMADDR->hmer2, HMADDR->hmcs3,
 HMADDR->hmda, HMADDR->hmca);
