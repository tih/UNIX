...
struct radevice {
	int	raip;		/* initialization and polling */
	int	rasa;		/* status and address */
};
...
typedef struct { unsigned low, high; } shorts;
...
struct raca {
	...
	int	ca_cmdint;	/* command queue transition interrupt flag */
	int	ca_rspint;	/* respoe queue transition interrupt flag */
...
struct ms {
	...
	struct {
		...
		int	Ms_hsize;	/* High order word of disk size */
		long	Ms_sftwds[4];	/* software words, padding */
	} ms_generic;
	struct {
		int	Ms_version;	/* MSCP version */
		int	Ms_cntflgs;	/* controller flags */
		int	Ms_hsttmo;	/* host timeout */
		int	Ms_usefrac;	/* use fraction */
		long	Ms_time;	/* time and date */
	} ms_setcntchar;
	struct {
		int	Ms_multunt;	/* multi-unit code */
		int	Ms_unitflgs;	/* unit flags */
		long	Ms_hostid;	/* host identifier */
		quad	Ms_unitid;	/* unit identifier */
		long	Ms_mediaid;	/* media type identifier */
		int	Ms_shdwunt;	/* shadow unit */
		int	Ms_shdwsts;	/* shadow status */
		int	Ms_track;	/* track size */
		int	Ms_group;	/* group size */
		int	Ms_cylinder;	/* cylinder size */
		int	Ms_xxx3;	/* reserved */
		int	Ms_rctsize;	/* RCT table size */
...
#define	ms_unt1		ms_un.ms_generic.Ms_dscptr
